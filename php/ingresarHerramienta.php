...
if ($row_duplicado['total'] > 0) {
    echo json_encode(["status" => "error", "mensaje" => "Ya existe una herramienta con el mismo nombre, marca y medida."]);
    exit;
}

// Preparar la consulta SQL
if ($ruta_imagen) {
    $sql = "INSERT INTO `herramienta`(`nombre`, `descripcion`, `stock_actual`, `id_marca`, `id_medida`, `imagen_path`) 
            VALUES ('$nombre','$descripcion','$stock','$id_marca','$id_medida','$ruta_imagen')";
} else {
    $sql = "INSERT INTO `herramienta`(`nombre`, `descripcion`, `stock_actual`, `id_marca`, `id_medida`) 
            VALUES ('$nombre','$descripcion','$stock','$id_marca','$id_medida')";
}
